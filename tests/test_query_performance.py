...
def benchmark_queries():
    ...
    log_console("=" * 100)
    log_console("PERFORMANCE BENCHMARK: Metadata Query vs Direct SELECT MIN/MAX")
    log_console("=" * 100)

    for meas, desc in test_cases:
        log_console(f"\n{'=' * 100}")
        log_console(f"Measurement: {meas}")
        log_console(f"Description: {desc}")
        log_console(f"{'=' * 100}\n")

        # TEST 1: Check if table has Parquet files
        log_console("[TEST 1] Checking Parquet file count...")
        try:
            ...
            log_console(f"  Parquet files: {file_count}")
        ...
        if valid_meta_times:
            ...
            log_console(f"  Times (ms): {', '.join(f'{t:.2f}' for t in valid_meta_times)}")
            log_console(f"  Average: {avg_meta:.2f} ms | Min: {min_meta:.2f} ms | Max: {max_meta:.2f} ms")
            if metadata_result:
                log_console(f"  Result: {metadata_result[0]} to {metadata_result[1]}")
        else:
            log_console(f"  No valid results (table likely has 0 Parquet files)")
            avg_meta = None
        ...
        if valid_direct_times:
            ...
            log_console(f"  Times (ms): {', '.join(f'{t:.2f}' for t in valid_direct_times)}")
            log_console(f"  Average: {avg_direct:.2f} ms | Min: {min_direct:.2f} ms | Max: {max_direct:.2f} ms")
            if direct_result:
                log_console(f"  Result: {direct_result[0]} to {direct_result[1]}")
        else:
            log_console(f"  All queries failed")
            avg_direct = None
        ...
        if avg_meta and avg_direct:
            ...
            log_console(f"  Metadata query: {avg_meta:.2f} ms (avg)")
            log_console(f"  Direct query: {avg_direct:.2f} ms (avg)")
            log_console(f"  Speedup ratio: {speedup:.2f}x (metadata is {speedup:.1f}x faster)")

            if avg_direct < 20:
                log_console(f"  CONCLUSION: Direct SELECT is FAST (<20ms) -> Likely uses metadata!")
            elif avg_direct < 100:
                log_console(f"  CONCLUSION: Direct SELECT is MODERATE (20-100ms) -> May use metadata + some scan")
            else:
                log_console(f"  CONCLUSION: Direct SELECT is SLOW (>100ms) -> Likely scans data!")
        elif avg_direct and not avg_meta:
            log_console(f"  Metadata query: FAILED (no Parquet files)")
            log_console(f"  Direct query: {avg_direct:.2f} ms (avg)")
            if avg_direct < 20:
                log_console(f"  CONCLUSION: Direct SELECT is FAST even without Parquet -> Uses WAL metadata!")
            else:
                log_console(f"  CONCLUSION: Direct SELECT scans WAL data")
        else:
            log_console(f"  CONCLUSION: Unable to compare (one or both queries failed)")

        # Result consistency check
        if metadata_result and direct_result:
            if metadata_result[0] == direct_result[0] and metadata_result[1] == direct_result[1]:
                log_console(f"  Results: IDENTICAL (metadata == direct)")
            else:
                log_console(f"  Results: DIFFERENT!")
                log_console(f"    Metadata: {metadata_result[0]} to {metadata_result[1]}")
                log_console(f"    Direct:   {direct_result[0]} to {direct_result[1]}")
                log_console(f"    REASON: Direct query sees WAL data that's not in Parquet yet")

    log_console("\n" + "=" * 100)
    log_console("BENCHMARK COMPLETE")
    log_console("=" * 100)
