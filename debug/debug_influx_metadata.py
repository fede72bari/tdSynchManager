...
def debug_metadata():
    ...
    log_console("=" * 80)
    log_console("DEBUG: InfluxDB v3 Parquet Metadata Investigation")
    log_console("=" * 80)
    ...
    try:
        ...
        if df is not None and not df.empty:
            log_console(f"[OK] system.parquet_files exists, has {df['cnt'].iloc[0]} rows")
        else:
            log_console("[EMPTY] system.parquet_files exists but is empty")
    except Exception as e:
        log_console(f"[ERROR] Failed to query system.parquet_files: {e}")
        return
    ...
    try:
        ...
        if df is not None and not df.empty:
            ...
            log_console(f"[OK] Found {len(tables)} tables:")
            for i, table in enumerate(tables[:10], 1):
                marker = "  <- TARGET" if table == test_meas else ""
                log_console(f"  {i}. {table}{marker}")
            if len(tables) > 10:
                log_console(f"  ... and {len(tables) - 10} more")
        else:
            log_console("[ERROR] No tables found in system.parquet_files")
            return
    except Exception as e:
        log_console(f"[ERROR] Failed to list tables: {e}")
        return
    ...
    try:
        ...
        if df is not None and not df.empty:
            ...
            log_console(f"  File count: {file_count}")
            if file_count > 0:
                ...
                log_console(f"  Earliest: {earliest}")
                log_console(f"  Latest: {latest}")
            else:
                log_console(f"[ERROR] No Parquet files found for '{test_meas}'")
        else:
            log_console("[ERROR] Query returned empty")
    except Exception as e:
        log_console(f"[ERROR] Failed to check Parquet files: {e}")
    ...
    try:
        ...
        if df is not None and not df.empty:
            ...
            if pd.notna(row['min_t']) and pd.notna(row['max_t']):
                ...
                log_console(f"[OK] Data exists in measurement:")
                log_console(f"  Min time: {min_t}")
                log_console(f"  Max time: {max_t}")
            else:
                log_console(f"[ERROR] Measurement exists but has no data")
        else:
            log_console(f"[ERROR] Measurement '{test_meas}' appears empty")
    except Exception as e:
        log_console(f"[ERROR] Failed to query measurement: {e}")
    ...
    try:
        ...
        df = result.to_pandas() if hasattr(result, "to_pandas") else result
        if df is not None and not df.empty:
            log_console(f"[OK] Columns: {df.columns.tolist()}")
            log_console(f"  Sample row:")
            for col, val in df.iloc[0].items():
                log_console(f"    {col}: {val}")
        else:
            log_console("[ERROR] Could not get schema (empty result)")
    except Exception as e:
        log_console(f"[ERROR] Failed to get schema: {e}")
